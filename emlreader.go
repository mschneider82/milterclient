...
func ReadMessage(r io.Reader) (msg *mail.Message, headers *MIMEHeaderOrdered, err error) {
	...
	hdr, hdr2, err := tp.ReadMIMEHeader()
	if err != nil {
		return nil, nil, err
	}
...
func (r *Reader) ReadMIMEHeader() (MIMEHeader, *MIMEHeaderOrdered, error) {
	...
	if buf, err := r.R.Peek(1); err == nil && (buf[0] == ' ' || buf[0] == '\t') {
		...
		return m, m2, textproto.ProtocolError("malformed MIME header initial line: " + string(line))
	}
	...
	for {
		...
		if i < 0 {
			return m, m2, textproto.ProtocolError("malformed MIME header line: " + string(kv))
		}
		endKey := i
		...
		// We could return a textproto.ProtocolError here, but better to be liberal in what we
		...
		m2.Keys = append(m2.Keys, key)
		m2.Values = append(m2.Values, value)
		if err != nil {
			return m, m2, err
		}
	}
